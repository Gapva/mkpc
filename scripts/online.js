function o_xhr(page, send, onload, backoff) {
  ...
  if (window.XMLHttpRequest || window.ActiveXObject) {
    ...
    } else xhr_object = new XMLHttpRequest();
  }
...
function o_inviteonline(memberID, battle) {
  o_xhr(
    "/invitations.php",
    "j0=" + memberID + (battle ? "&battle" : ""),
    function (reponse) {
      if (reponse == 1) {
        var o_msgId = displayMsg(
          o_language ? "Invitation sent" : "Invitation envoy&eacute;e",
          false
        );
        if (o_msgId != -1) {
...
function o_initdataset(o_div) {
  if (!o_div.dataset) o_div.dataset = {};
}
  ...
  if (o_table) {
    ...
    if (oListJoueurs.length) {
      ...
      for (var i = 0; i < oListJoueurs.length; i++) {
        ...
        if (oIgnores.indexOf(oJoueur[0]) != -1) o_selector.dataset.ignored = 1;
        ...
        if (oJoueur[3]) {
          ...
          o_flag.onload = function () {
            this.style.top =
              Math.round(
                (o_selector.offsetHeight - this.offsetHeight - 1) / 2
              ) + "px";
        ...
        o_selector.onmouseup = function (e) {
          ...
          if (this.dataset.action == 0) {
            ...
            if (this.dataset.ignored == 1) {
              ...
              o_unignore.innerHTML = o_language
                ? "Unignore"
                : "D&eacute;signorer";
              ...
              o_profile.innerHTML = o_language
                ? "See profile"
                : "Voir le profil";
              if (!o_profile.dataset) o_profile.dataset = {};
              ...
              o_ignore.onclick = function (e) {
                o_confirm(
                  o_language
                    ? "Ignore " +
                        memberPseudo +
                        "?<br />You won't be able to send or receive messages from him."
                    : "Ignorer " +
                        memberPseudo +
                        " ?<br />Vous ne pourrez plus envoyer ni recevoir de messages de lui.",
                  function (res) {
                    if (res) {
                      o_xhr(
                        "/ignore.php",
              ...
              if (o_invite) o_ctxtmenu.appendChild(o_invite);
              o_ctxtmenu.appendChild(o_talk);
            }
            o_liste.appendChild(o_ctxtmenu);
            o_ctxtmenu_y =
              o_selector.offsetTop -
              o_table.scrollTop +
              10 -
              o_ctxtmenu.scrollHeight;
      ...
      o_table.innerHTML = o_language
        ? "Nobody is currently connected"
        : "Aucun joueur connect&eacute; pour l'instant";
    }
  }
}
...
function o_fill_chats(oMask, lastMembers) {
  ...
  oH1.innerHTML =
    (o_language ? "All my conversations" : "Toutes mes conversations") +
    " (" +
    lastMembers.length +
    ")";
  ...
  if (lastMembers.length) {
    ...
    for (var i = 0; i < lastMembers.length; i++) {
      ...
      if (lastMember[4] == 0) oMsg.className = "o_msg_new";
      oMsg.innerHTML = lastMember[2];
      oConv.appendChild(oMsg);
      if (!oConv.dataset) oConv.dataset = {};
      oConv.dataset.i = i;
  ...
  oForm.onsubmit = function () {
    ...
    if (pseudo) {
      ...
      o_xhr("/findIDByPseudo.php", "pseudo=" + pseudo, function (memberID) {
        ...
        if (memberID == -1)
          o_alert(
            o_language ? "This nick doesn't exist" : "Ce pseudo n'existe pas.",
            redoChat
          );
        else if (memberID == -2) o_launchchat(o_id, pseudo);
        else o_launchchat(memberID, pseudo);
        return true;
      });
    }
    return false;
...
function o_invitesb() {
  ...
  if (!document.getElementById("coliste")) {
    ...
    o_infos.innerHTML = o_language
      ? 'Chat with other members or invite them to play with you in the <a href="/online.php" target="_blank">online mode</a>.'
      : 'Ici, discutez avec les autres membres, ou invitez-les &agrave; vous affronter dans le <a href="/online.php" target="_blank">mode en ligne</a>.';
    ...
    o_decochat.innerHTML = o_language
      ? '<a href="javascript:o_my_chats()">All my conversations</a>'
      : '<a href="javascript:o_my_chats()">Toutes mes conversations</a>';
...
function o_inserttags(msg) {
  ...
  msg = msg.replace(
    /([a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4})/gi,
    '<a href="mailto:$1">$1</a>'
  );
  var smileyNames = [
    "mdr",
  ...
  for (var i = 0; i < smileysList.length; i++) {
    var newStr =
      '<img src="/images/smileys/smiley' +
      i +
      '.png" class="osmiley" alt="Smiley" />';
    msg = msg.split(smileysList[i]).join(newStr);
    msg = msg.split(smileysList[i].toLowerCase()).join(newStr);
  }
  msg = msg.replace(
    new RegExp("\\:([1-3]?\\d|4[0-0])\\:", "g"),
    '<img src="/images/smileys/smiley$1.gif" alt="Smiley $1" />'
  );
  return msg;
}

function o_todate(sqlDate) {
  var ymdhi = /(\d{4})\-(\d{2})\-(\d{2}) (\d{2}):(\d{2}):(\d{2})/g.exec(
    sqlDate
  );
  return new Date(
    ymdhi[1],
...
function o_tohumandate(d) {
  ...
  var isToday =
    ...
    now.getYear() == d.getYear();
  var strHour = d.getHours() + ":" + o_zerofill(d.getMinutes());
  if (isToday) return strHour;
  else if (o_language)
    return (
      o_zerofill(d.getMonth() + 1) +
      "-" +
      o_zerofill(d.getDate()) +
      "," +
      strHour
    );
  else
...
function o_addmsg(memberID, msgData, toBeginning) {
  ...
  o_msg.className =
    "chatmsg " + (sender == memberID ? "coreceiver" : "cosender");
  o_initdataset(o_msg);
  ...
  if (toBeginning) o_list.insertBefore(o_msg, o_list.firstChild);
  else o_list.appendChild(o_msg);
  return o_msg;
}
function o_insertmsgs(memberID, messages) {
  ...
  if (messages.length < MSGS_PACKET_SIZE) {
    ...
    if (o_list.scrollTop < MSGS_PADDINGTOP) scrollMore -= MSGS_PADDINGTOP;
    setTimeout(function () {
...
function o_appendmsgs(memberID, messages) {
  var o_chat = document.getElementById("chatwindow" + memberID);
  var o_list = o_chat.getElementsByClassName("chatmsgs")[0];
  var isBottomScrolled =
    o_list.scrollTop >=
    o_list.scrollHeight - o_list.clientHeight - SCROLL_MARGIN;
  var lastID = o_lastmsgid(o_chat);
  var isNewMsg = false;
  for (var i = 0; i < messages.length; i++) {
    if (messages[i][0] > lastID) {
      var o_msg = o_addmsg(memberID, messages[i], false);
      if (messages[i][1] == memberID) isNewMsg = true;
      else isNewMsg = false;
    }
  }
  if (isNewMsg) {
    if (!o_focused) {
      displayTitle(
        o_language
          ? "New message from " + o_chat.dataset.pseudo
          : "Nouveau message de " + o_chat.dataset.pseudo
      );
      o_newmsgsound();
    }
  }
  if (messages.length) o_updatechatactivity(o_chat);
  if (isBottomScrolled) o_list.scrollTop = o_list.scrollHeight;
}
...
function o_notify_online(pseudo, battle) {
  if (!o_focused)
    b_notify(
      pseudo,
      o_language
        ? "Propose an online " + (battle ? "battle" : "race") + " with you."
        : "Propose de vous affronter sur une " +
            (battle ? "bataille" : "course") +
            " en ligne."
    );
}
function o_newmsgsound() {
  if (o_active) {
    if (!o_focused) {
      ...
      if (pEmbed.length) document.body.removeChild(pEmbed[0]);
...
function o_lastmsgid(o_chat) {
  var o_msgs = o_chat.getElementsByClassName("chatmsg");
  if (o_msgs.length) return o_msgs[o_msgs.length - 1].dataset.id;
  return 0;
}
...
function o_updateinputsize(o_input) {
  ...
  if (o_input.value) o_input.style.height = o_input.scrollHeight + 4 + "px";
}
function o_startchat(memberID, pseudo, messages, autofocus) {
  var o_chat = document.getElementById("chatwindow" + memberID);
  if (o_chat) {
    if (o_chat.dataset.reduced) o_restorechat(o_chat);
    return;
  }
  ...
  o_title.innerHTML =
    '<span class="chatbullet">\u2022</span><a href="/profil.php?id=' +
    memberID +
    '">' +
    pseudo +
    "</a>";
    ...
    if (answerValue) {
      ...
      o_xhr(
        ...
        function (reponse) {
          ...
          o_chat.dataset.typing = o_chat.getElementsByTagName("textarea")[0]
            .value
            ? 1
            : 0;
          return true;
  ...
  if (autofocus) o_input.focus();
}
function o_stopchat(memberID) {
  var o_chat = document.getElementById("chatwindow" + memberID);
  if (o_chat) o_closechat(o_chat);
}
function o_closechat(o_chat) {
  o_chats_ctn.removeChild(o_chat);
  if (o_chats.indexOf) o_chats.splice(o_chats.indexOf(o_chat), 1);
}
...
function o_updatestatut(memberID) {
  var o_chat = document.getElementById("chatwindow" + memberID);
  if (o_chat) o_changestatut(o_chat);
}
...
function o_isconnected(memberID) {
  if (memberID == o_id) return true;
  for (var i = 0; i < oListJoueurs.length; i++) {
    if (oListJoueurs[i][0] == memberID) return true;
  }
  return false;
}
function o_getstatus(memberID) {
  if (memberID == o_id) return o_online + o_active;
  for (var i = 0; i < oListJoueurs.length; i++) {
    if (oListJoueurs[i][0] == memberID) return oListJoueurs[i][2];
  }
  return 0;
}
...
function close_jco(o_msgId) {
  ...
  for (var i = o_msgId + 1; document.getElementById("comsg" + i); i++) {
    document.getElementById("comsg" + i).getElementsByTagName("a")[0].href =
      "javascript:close_jco(" + (i - 1) + ")";
...
function setDecoLink() {
  document.getElementById("statutco").className = o_online
    ? o_active
      ? "isconnected"
      : "ismuted"
    : "isnotconnected";
...
function displayMsg(m, changetitle) {
  if (o_active) {
    ...
    o_close.className = "closelister";
    o_close.innerHTML =
      '<a href="javascript:close_jco(' + o_msgId + ')">&times;</a>';
...
function o_loadOnline() {
  ...
  o_xhr("/getpseudo.php", "", function (reponse) {
    if (reponse) {
      ...
      if (!oPseudoInfos.length) return true;
      o_id = oPseudoInfos[0];
      ...
      o_div.id = "connect";
      o_div.innerHTML =
        '<div id="pseudostatus"><a title="' +
        (o_language ? "See my profile" : "Voir mon profil") +
        '" href="/profil.php?id=' +
        oPseudoInfos[0] +
        '">' +
        oPseudoInfos[1] +
        '</a> : <select id="statutco" class="isnotconnected" onchange="o_deco()"><option value="0" class="isnotconnected">' +
        (o_language ? "Offline" : "Hors-ligne") +
        '</option><option value="1" class="ismuted">' +
        (o_language ? "Do not disturb" : "Ne pas d&eacute;ranger") +
        '</option><option value="2" class="isconnected">' +
        (o_language ? "Connected" : "Connect&eacute;") +
        '</option></select></div><a class="coselectable" href="javascript:o_invitesb()">' +
        (o_language ? "Members Area" : "Espace membres") +
        '</a><a class="coselectable" href="https://discord.gg/VkeAxaj" target="_blank">' +
        (o_language ? "Discord Server" : "Serveur Discord") +
        "</a>";
      ...
      o_xhr("/coreload.php", "", function (reponse) {
        if (reponse) {
          ...
          for (var i = 0; i < oListJoueurs.length; i++) {
            var o_chat = document.getElementById(
              "chatwindow" + oListJoueurs[i][0]
            );
            if (o_chat) {
              o_changestatut(o_chat);
              o_updatechatactivity(o_chat);
...
function o_unloadOnline() {
  if (!o_online) {
    ...
    for (var i = o_chats.length - 1; i >= 0; i--) o_closechat(o_chats[i]);
    if (document.getElementById("coliste")) o_closelist();
  }
  if (!o_active) {
    close_allco();
    oConvs.length = 0;
...
function inArray0(a, elt) {
  for (var i = 0; i < a.length; i++) {
    if (a[i][0] == elt) return true;
  }
  return false;
}
function indexOf0(a, elt) {
  for (var i = 0; i < a.length; i++) {
    if (a[i][0] == elt) return a[i];
  }
  return null;
}
function inArray03(a, elt) {
  for (var i = 0; i < a.length; i++) {
    if (a[i][0] == elt[0] && a[i][3] == elt[3]) return true;
  }
  return false;
}
function inArray04(a, elt) {
  for (var i = 0; i < a.length; i++) {
    if (a[i][0] == elt[0] && a[i][4] == elt[4]) return true;
  }
  return false;
}
...
function o_dialog() {
  ...
  oMask.onclick = oCross.onclick = function () {
    document.body.removeChild(oMask);
    if (document.onkeydown == nKeydown) document.onkeydown = aKeydown;
    return false;
  };
  oDialog.appendChild(oCross);
  var oMessage = document.createElement("div");
...
function o_repond(dem, name, rep, elt, battle) {
  o_prompt(
    rep
      ? o_language
        ? "Send an additionnal message to " + name + " (optionnal)"
        : "Envoyer un message en compl\xE9ment (facultatif)"
      : o_language
      ? "Send a message to explain your refusal (optional)"
      : "Envoyer un message pour expliquer votre refus (facultatif)",
    o_language
...
function o_disappear(o_msg, time) {
  ...
  function o_fadeout(o_msg) {
    if (document.body.contains(o_msg)) {
      ...
      } else eval(o_msg.getElementsByTagName("a")[0].href);
    }
  }
    ...
    function (reponse) {
      if (reponse == 1) {
        if (rep)
          displayMsg(
            o_language
              ? 'Invitation accepted! <a href="/online.php' +
                  (battle ? "?battle" : "") +
                  '">Click here</a> to go to the online mode!'
              : 'Demande accept&eacute;e&nbsp;! <a href="/online.php' +
                  (battle ? "?battle" : "") +
                  '">Cliquez ici</a> pour acc&eacute;der au mode en ligne&nbsp;!',
            false
          );
        return true;
      }
...
function deleteCross(elt) {
  for (var i = 0; document.getElementById("comsg" + i); i++) {
    var o_inputs = document
      .getElementById("comsg" + i)
  ...
  o_xhr("/coreload.php", data.join("&"), function (reponse) {
    if (reponse) {
      ...
      for (var i = 0; i < oJoueursInfos[0].length; i++) {
        ...
        if (!currentPlayer) newPlayers.push(oJoueursInfos[0][i]);
        ...
        var o_msgId = displayMsg(
          o_language
            ? "<b>" +
                ...
                "</b> (" +
                newDemandes[i][2] +
                " pts) proposes an online <b>" +
                (newDemandes[i][3] ? "battle" : "race") +
                '</b> with you<br /><input type="button" value="Accept" onclick="o_send_answer(' +
                newDemandes[i][0] +
                ",1,''," +
                newDemandes[i][3] +
                ');deleteCross(this)" /> - <input type="button" value="Reject" onclick="o_repond(' +
                newDemandes[i][0] +
                ",'" +
                newDemandes[i][1] +
                "',0,this," +
            ...
            : "<b>" +
                ...
                "</b> (" +
                newDemandes[i][2] +
                " pts) propose de vous affronter sur une <b>" +
                (newDemandes[i][3] ? "bataille" : "course") +
                '</b> en ligne<br /><input type="button" value="Accepter" onclick="o_send_answer(' +
                newDemandes[i][0] +
                ",1,''," +
                newDemandes[i][3] +
                ');deleteCross(this)" /> - <input type="button" value="Refuser" onclick="o_repond(' +
                newDemandes[i][0] +
                ",'" +
                newDemandes[i][1] +
                "',0,this," +
      ...
      if (newDemandes.length) o_newmsgsound();
      ...
      for (var i = 0; i < newReponses.length; i++) {
        ...
        var o_msgId = displayMsg(
          newReponses[i][2]
            ? o_language
              ? newReponses[i][1] +
                " <b>accepted</b> your invitation" +
                (newReponses[i][3]
                  ? ' with the following messaage: <p class="o_msg"><span>' +
                    newReponses[i][3] +
                    "</span></p>"
                  : "! ") +
                '<a href="/online.php' +
                (newReponses[i][4] ? "?battle" : "") +
                '">Click here</a> to go to the online mode!'
              : newReponses[i][1] +
                "</b> a <b>accept&eacute;</b> votre demande" +
                (newReponses[i][3]
                  ? ' avec le message suivant&nbsp;: <p class="o_msg"><span>' +
                    newReponses[i][3] +
                    "</span></p>"
                  : "&nbsp;! ") +
                '<a href="/online.php' +
            ...
            ? "Sorry, " +
              newReponses[i][1] +
              " <b>rejected</b> your invitation" +
              (newReponses[i][3]
                ? ' for the following reason: <p class="o_msg"><span>' +
                  newReponses[i][3] +
                  "</span></p>"
                : "...")
            : "Dommage, " +
              newReponses[i][1] +
              " a <b>refus&eacute;</b> votre demande" +
              (newReponses[i][3]
                ? ' pour la raison suivante&nbsp;: <p class="o_msg"><span>' +
                  newReponses[i][3] +
                  "</span></p>"
                : "..."),
          true
      ...
      if (!o_handler) o_handler = setTimeout(o_chatrefresh, 1000);
      return true;
    }
    return false;
  });
...
function o_chatrefresh() {
  ...
  if (o_online) {
    o_refreshstate++;
    if (o_refreshstate >= (o_active ? 5 : 10)) o_refreshstate = 0;
    if (!o_refreshstate) return o_refresh();
    var data = new Array();
...
function o_handlenewconvs(newConvs) {
  ...
  for (var i = 0; i < newConvs.length; i++) {
    ...
    var o_msgId = displayMsg(
      o_language
        ? "New message from <strong>" +
            newConv[2] +
            '</strong>:<p class="o_msg"><span>' +
            newConv[3] +
            '</span></p><input type="button" class="o_ansnew" value="Answer" />'
        : "Nouveau message de <strong>" +
            newConv[2] +
            '</strong>&nbsp;:<p class="o_msg"><span>' +
            newConv[3] +
            '</span></p><input type="button" class="o_ansnew" value="R&eacute;pondre" />',
      true
    );
    if (o_msgId != -1) {
      var o_msg = document.getElementById("comsg" + o_msgId);
  ...
  if (newConvs.length) o_newmsgsound();
}
  ...
  for (var i = 0; i < oWritting.length; i++) {
    ...
    if (o_chat) {
      if (o_chat.className.indexOf("chat_isw") == -1) {
        ...
        if (scrolledToBottom) o_list.scrollTop = o_list.scrollHeight;
      }
    }
  }
  for (var i = 0; i < o_chats.length; i++) {
...
function o_updateactivityto(memberID, timestamp) {
  if (oLastActivities[memberID] >= timestamp) return;
...
function waitForLoad() {
  if (document.body) o_loadOnline();
  else setTimeout(waitForLoad, 1000);
}
waitForLoad();

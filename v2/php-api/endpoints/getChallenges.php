...
while ($challenge = mysql_fetch_array($getChallenges)) {
	...
	if ($completed) {
		if (isset($challenge['avgrating']) && isset($challenge['nbratings'])) {
			$challengeData['rating'] = array(
				'avg' => +$challenge['avgrating'],
				'nb' => +$challenge['nbratings']
			);
		}
	}
	elseif (isset($challengeDetails['rating']))
